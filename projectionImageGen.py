#!/usr/bin/env python3
"""Generate custom text-format .ry ray files from a TOML projection configuration.
...
def _write_ry(
	...
	with path.open("w", encoding="utf-8") as f:
		f.write("# ray file v1\n")
		f.write(f"# generated by projectionImageGen.py v{__version__}\n")
...
def run(config_path: Path) -> None:
	...
	for batch_idx in range(cfg.output.batch_count):
		...
		_write_ry(out_path, origins, directions, energies, config_path)
...
def validate_config(config_path: Path) -> RayGeneratorConfig:
	cfg = load_config(config_path)
	mask = _build_mask(cfg)
	...
	if cfg.emission.origin_type == "pixelated":
		if cfg.pixel is None:
			raise ValueError("[pixels] table is required for emission.origin_type=pixelated")
		_build_pixel_centers(cfg.projection.width, cfg.pixel.pitch[0])
		_build_pixel_centers(cfg.projection.height, cfg.pixel.pitch[1])
		_map_bin_size(cfg, mask, 1)
	else:
		if cfg.continuous is None:
			raise ValueError("[continuous] table is required for emission.origin_type=continuous")
		_map_bin_size(cfg, mask, cfg.continuous.n_rays)
...
def parse_args() -> argparse.Namespace:
	...
	parser.add_argument(
		...
		dest="validate_only",
		action="store_true",
		help="Load and validate config without generating output files",
	)
	return parser.parse_args()


def _format_config_error(config_path: Path, exc: Exception) -> str:
	if isinstance(exc, FileNotFoundError):
		return f"Config file not found: {config_path}"
	if isinstance(exc, FileExistsError):
		return f"Output file conflict while processing '{config_path}': {exc}"
	if isinstance(exc, OSError):
		return f"I/O error while processing '{config_path}': {exc}"
	if isinstance(exc, ValueError):
		return f"Configuration error in '{config_path}': {exc}"
	if isinstance(exc, RuntimeError):
		return f"Runtime setup error for '{config_path}': {exc}"
	return f"Failed while processing '{config_path}': {exc}"
...
def main() -> None:
	...
	try:
		if args.validate_only:
			...
			print(f"Config validation successful: {config_path}")
			print(
				f"Emission={cfg.emission.origin_type}, Focus={cfg.focus.mode}, Mask={cfg.mask.type}, "
				f"Batches={cfg.output.batch_count}"
			)
			return

		run(config_path)
	except (ValueError, RuntimeError, OSError, FileNotFoundError, FileExistsError) as exc:
		raise SystemExit(_format_config_error(config_path, exc)) from exc
